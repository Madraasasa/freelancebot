...
def insert_task(name, desc, sum, sex,age, tasker_dt, t_date, level):
    try:
        con = p.connect(database='bot', user='postgres', host='localhost', password='VivaProgressio', port=5433)
        cur = con.cursor()
        list_age = age.split('-')
        cur.execute('select count(*) from public.task')
        count = cur.fetchone()
        cur.close()
        cur = con.cursor()
        cur.execute(f'''INSERT INTO public.task(id, name, "desc", sum, sex, ages, agee, tasker_d, task_date, count_t, level) 
VALUES ({int(list(count)[0])+1},'{name}', '{desc}', {sum}, '{sex}', {list_age[0]}, {list_age[1]}, {tasker_dt}, {t_date}, 0, '{level}')''')
        con.commit()
        con.close()
...
# insert_task('test_name1', 'test_name2', 100, 'Ж', '18-22','5','7', 'medium')
...
#
# def insert_task1(name, desc, sum, sex,age, tasker_dt, t_date, level):
#     con = p.connect(database='bot', user='postgres', host='localhost', password='VivaProgressio', port=5433)
#     cur = con.cursor()
#     list_age = age.split('-')
#     cur.execute('select count(*) from public.task')
